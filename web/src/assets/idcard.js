export const cardUtil = {
  ...
  validateIdCard(idCard) {
    ...
    //校验生日
    this.bir = this.checkBirthday(idCard);
    if (!this.bir) {
      return false;
    }
    //检验位的检测
  ...
  checkBirthday(card) {
    ...
    if (len == '15') {
      ...
      var day = arr_data[4];
      this.birthday = new Date('19' + year + '/' + month + '/' + day);
      if (this.verifyBirthday('19' + year, month, day, this.birthday)) {
        bir = ('19' + year + '-' + month + '-' + day);
        sexNumber = card.substring(14, 15);
      }
      ...
      if (this.verifyBirthday(year, month, day, this.birthday)) {
        bir = (year + "-" + month + "-" + day);
        sexNumber = card.substring(16, 17);
      }
    }
    if(sexNumber % 2 != 0) {
      this.sex = 1;
